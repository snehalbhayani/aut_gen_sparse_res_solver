function[] = test_synthetic_solver_data()
% clc;
folder_name = 'sp_solvers/problem_p5p_refractive';
problem_name = 'problem_p5p_refractive';
addpath(folder_name);
addpath('eqs');
...
load('../scene_generator/problem_p5p_refractive.mat');
load('../scene_generator/gt_p5p.mat');
...
for index = 1:5000
    ...
    for i = 1:noofpepsolns
        sol = PEPSolutions(:,i);
        try
            r = [r, norm(abs([1;abs(sol([4,5,1]))] - [abs(gt(1:4))]))/norm(abs(gt(1:4))) ];
            t = [t, norm(abs([sol([2,3])] - t_gt(1:2)))/norm(t_gt) ];
        catch
            disp("Some error");
        end
    end
    ...
    if (mod(index,1)==0)
        ...
        try
            ...
            temp = mat2cell([transpose(closestsol([2,3,4,5,1])),data],1,1*ones(1,length(data)+length(closestsol)));
            eqs = Eqs_red_problem_p5p_refractive(temp{:});
            
            all_residual = [all_residual, max(abs(eqs))/norm(closestsol)];
            min_gt_error = [min_gt_error, [min(abs(sol(1,:))); min(abs(sol(2,:)))]];
